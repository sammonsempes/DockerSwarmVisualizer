...
<body>
        ...
        <div class="main-content">
            <aside class="sidebar">
                ...
                <div class="legend">
                    ...
                    <div class="legend-item"><div class="legend-line" style="background: var(--terminal-orange);"></div><span>Service → Node</span></div>
                    <div class="legend-item"><div class="legend-line" style="background: var(--terminal-green);"></div><span>Service → Network</span></div>
                </div>
            </aside>
            <main class="visualization">
                ...
                <div class="zoom-controls">
                    ...
                    <button class="zoom-btn" id="zoom-reset" title="Reset View" style="font-size: 1rem;">⟲</button>
                </div>
            </main>
        </div>
    </div>
    <script>
        ...
        function displayTooltipContent(d) {
            ...
            if (d.type === 'node') {
                content = `
                    <div class="tooltip-section"><div class="tooltip-label">Role</div><div class="tooltip-value">${d.role === 'manager' || d.role === 'Leader' || d.role === 'Reachable' ? '👑 Manager' : '⚙️ Worker'}</div></div>
                ...
                content = `
                    <div class="tooltip-section"><div class="tooltip-label">Type</div><div class="tooltip-value">🌐 Overlay Network</div></div>
                ...
                if (d.networks && d.networks.length > 0) {
                    ...
                    if (d.networks.length > 3) {
                        ...
                        networksHtml = visible + '<span class="tooltip-expand" data-target="networks">▼ +' + (d.networks.length - 3) + ' more</span><div class="tooltip-hidden" id="networks-hidden">' + hidden + '</div>';
                    } else {
                ...
                if (d.mounts && d.mounts.length > 0) {
                    const visible = d.mounts.slice(0, 2).map(m => '<div>📁 '+m+'</div>').join('');
                    if (d.mounts.length > 2) {
                        const hidden = d.mounts.slice(2).map(m => '<div>📁 '+m+'</div>').join('');
                        mountsHtml = `<div class="tooltip-section"><div class="tooltip-label">Mounts (${d.mounts.length})</div><div class="tooltip-value" style="font-size: 0.65rem;">${visible}<span class="tooltip-expand" data-target="mounts">▼ +${d.mounts.length - 2} more</span><div class="tooltip-hidden" id="mounts-hidden">${hidden}</div></div></div>`;
                    } else {
                ...
                if (d.labels && Object.keys(d.labels).length > 0) {
                    ...
                    if (entries.length > 3) {
                        ...
                        labelsHtml = `<div class="tooltip-section"><div class="tooltip-label">Labels (${entries.length})</div><div class="tooltip-value" style="font-size: 0.65rem;">${visible}<span class="tooltip-expand" data-target="labels">▼ +${entries.length - 3} more</span><div class="tooltip-hidden" id="labels-hidden">${hidden}</div></div></div>`;
                    } else {
                ...
                content = `
                    ...
                    <div class="tooltip-section"><div class="tooltip-label">Image</div><div class="tooltip-value" style="font-size: 0.7rem;">🐳 ${d.image || 'N/A'}</div></div>
        ...
        document.getElementById('tooltip').addEventListener('click', (e) => {
            ...
            if (e.target.classList.contains('tooltip-expand')) {
                ...
                if (hiddenEl && hiddenEl.classList.contains('tooltip-hidden')) {
                    ...
                    e.target.textContent = isExpanded ? '▲ See less' : '▼ +' + count + ' more';
                }
            }
        });
